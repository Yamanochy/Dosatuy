...
function renderStatistics() {
  ...
  let totalTrips = 0, totalMoney = 0, totalWeight = 0;
  Object.values(monthData).forEach((r) => {
    totalTrips += r.shifts.length;
    totalMoney += r.shiftPay + r.maintPay;
  });
  docsCache.forEach((d) => {
  ...
  const statsRow = el("div", "grid grid-cols-3 gap-2");
  statsRow.appendChild(statCard("рейсов", totalTrips, "🚛"));
  statsRow.appendChild(statCard("заработано", totalMoney.toLocaleString("ru-RU") + " ₽", "💰"));
  ...
  trendCard.innerHTML = `<div class="font-bold text-slate-700 mb-2 text-sm">Рейсов по месяцам</div><canvas id="chart-trend" height="150"></canvas>`;
  wrap.appendChild(trendCard);

  const truckCard = el("div", "bg-white rounded-2xl shadow-sm p-4");
  truckCard.innerHTML = `<div class="font-bold text-slate-700 mb-2 text-sm">Грузовики в этом месяце</div><canvas id="chart-trucks" height="120"></canvas>`;
  ...
  setTimeout(() => {
    ...
    if (ctxTrucks && typeof Chart !== "undefined") {
